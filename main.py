import requests
import time
import signal
import sys
import concurrent.futures
import os
import random
from collections import OrderedDict
import urllib.parse

# ================== CREDENTIALS (User Provided - Exposed, Use at Own Risk) ==================
X_ACCESS_TOKEN = ""

ADDRESS_ID = "0228a3439c5d0108711a7ee829c60b8e09f9ce168844e125d211f0bf05b7956f"

DEVICE_ID = "8455FFF3-8E05-4F50-8087-D71D37FB4F2C"

X_ACF_SENSOR_DATA = "4,i,oKaBIsIWpVocxqq18CbaPiy1yqM7GyXKB/8zjL5D8Vy1PQ8b06VlgMIiDi/s9DU7mpLXELXe4PEEQhtgUZ/Szhz9ZbF7uw5k5j+9ZFiq4riGr86A8JDvQ/mLw4pPXMoC31GGkYhXF6BvZAT7ETlqZ6nNNI4E8o6pNksA1vm78c4=,VFRSAygDR0ml21S0/gAki5C7FJaJz6pTu9JjcScg4evsU33+CfeBWHkzijyyydJ1n4nY+fsc7l2OYad1iV2QzcCY0ItnlQlHtOJ8iNGDZ2T6PGf04GHxrCGXvMoaGSZMvZTZEVQ/oj11zhNYePptXEJtFL1D01CL08MB3FtSZEY=$$15,10,57$$$AAQAAAAG%2f%2f%2f%2f%2f6w12nf0j0t7AZEnrpm6qREwRI0OFbAwCv6VRm4Dxj%2fbyWx56x423MbT01hogTeFLIlVTLwIK1CNCr6TD9AthqP7l4tWiX2Kx3vCTWfthC%2fhVGU5jz5vWZ4rA27WOcosuHCRkZaoybnIffOVYg0SKwwtuzLIsAVGq7PXuGEdNvBHb+zQWMPaZZBVfO9ws59PkFLY2+QT6viFis653PaOn20iwXfGEzB8YxT6iWEJ%2fM0YPoXEnq2DeBjtM61RTb6v6ocsZmqldCqeGOJBBrWrVkuWq2XdibUBrx%2fuy3Tcw5oBZZgDw8D7hlBwemxKGCsZC1PCQbimuntAiMgkKGGvlbw%3d"

COOKIE = "_abck=FAE3E3D938D02209ECBA22E43979144B~-1~~-1~-1~1787322191~AAQAAAAG%2f%2f%2f%2f%2f8Ulr3TB7htCa1Y95KdDle7jLmdMrCtZWGwsahR0vAX17oyh6t14H3jBI1px8AUsO96k%2fKV2wybtIhkqQ1C+uQa4S%2fgZ2bWoNN4s~-1; ak_bmsc=6C173AB8D753C2362D8FE5DB16C691F6~000000000000000000000000000000~; _ga_6XHS1RXTBF=GS2.1.s1787318723$o1$g1$t1787318724$j59$l0$h0; _ga_VFNMV2T18E=GS2.1.s1787318640$o1$g0$t1787318643$j57$l0$h0; bm_mi=E616A45095D2AEE0A0070D737827D17A~~1; bm_sv=75A027D1E600195E452F4ED8E98661E4~~1; bm_sz=785EA17CBD681B4A5D4F748E2A0C2BD5~~4469045~3359302; aopopuwtjtssi=RS_10_10_24_81_BJP_80; _ga_HQGP5S5CDY=GS2.1.s1786947916$o1$g0$t1786947916$j60$l0$h0; _ga_D18QMBPWTR=GS2.1.s1787320424$o1$g0$t1787320431$j53$l0$h0; _ga=GA1.1.1634634630.1786947916; _ga_W68WP061GD=GS2.1.s1787320342$o1$g1$t1787320387$j15$l0$h0"

# ================== CONFIGURATION ==================
def get_config(key, default):
    return os.getenv(key, default)

START_POST_ID = int(get_config("START_POST_ID", "50000"))
END_POST_ID = int(get_config("END_POST_ID", "1000000000000000000"))
CONCURRENT_WORKERS = int(get_config("CONCURRENT_WORKERS", "5"))
DELAY_PER_REQUEST = float(get_config("DELAY_PER_REQUEST", "0.5"))
PAUSE_INTERVAL = int(get_config("PAUSE_INTERVAL", "30"))
PAUSE_DURATION = float(get_config("PAUSE_DURATION", "3"))
REQUEST_TIMEOUT = int(get_config("REQUEST_TIMEOUT", "30"))
MAX_RETRIES = int(get_config("MAX_RETRIES", "3"))
MAX_CONSECUTIVE_403 = int(get_config("MAX_CONSECUTIVE_403", "5"))

USE_PROXY = get_config("USE_PROXY", "False").lower() == "true"
PROXY_URL = get_config("PROXY_URL", "http://IP:PORT")

API_URL = get_config("API_URL", "https://api.narendramodi.in/apiv1")

# ================== HEADERS (match captured request order) ==================
headers_template = OrderedDict([
    ("Host", "api.narendramodi.in"),
    ("Content-Type", "application/x-www-form-urlencoded"),
    ("Accept-Encoding", "gzip, deflate"),
    ("X-acf-sensor-data", X_ACF_SENSOR_DATA),
    ("Cookie", COOKIE),
    ("Connection", "close"),
    ("Accept", "*/*"),
    ("User-Agent", "Narendra Modi App/7.9 (iOS 18.5; iPhone Build/Narendra Modi App)"),
    ("requestFrom", "ios"),
    # Content-Length will be added dynamically before sending
    ("Accept-Language", "en-IN,en;q=0.9"),
])

# ================== GRACEFUL SHUTDOWN ==================
shutdown_requested = False

def signal_handler(sig, frame):
    global shutdown_requested
    print("\n⚠️  Shutdown requested. Completing current tasks...")
    shutdown_requested = True

signal.signal(signal.SIGINT, signal_handler)
signal.signal(signal.SIGTERM, signal_handler)

# ================== REQUEST FUNCTION ==================
def send_request_with_retry(post_id, session, consecutive_403_counter):
    global shutdown_requested

    if shutdown_requested:
        return False

    for attempt in range(1, MAX_RETRIES + 1):
        if shutdown_requested:
            return False

        try:
            # Build payload as OrderedDict to preserve order (same as captured)
            payload = OrderedDict([
                ("postid", str(post_id)),
                ("actionon", "nmreels"),
                ("type", "nmreels"),
                ("flag", "1"),
                ("action", "postlikedislike"),
                ("deviceid", DEVICE_ID),
                ("X-Access-Token", X_ACCESS_TOKEN),
                ("addressid", ADDRESS_ID),
                ("x-app-version", "7.9"),
                ("apiversion", "2"),
                ("navigationtag", ""),
            ])

            # Encode body to string
            body_string = urllib.parse.urlencode(payload)

            # Prepare headers
            current_headers = headers_template.copy()
            # Set Content-Length dynamically
            current_headers["Content-Length"] = str(len(body_string))

            proxies = None
            if USE_PROXY:
                proxies = {"http": PROXY_URL, "https": PROXY_URL}

            # Random jitter before request
            time.sleep(random.uniform(0, 0.5))

            response = session.post(
                API_URL,
                data=body_string,   # pass string, not dict, to avoid auto-encoding
                headers=current_headers,
                timeout=REQUEST_TIMEOUT,
                proxies=proxies,
            )

            if response.status_code == 200:
                try:
                    json_response = response.json()
                    status = json_response.get("status", "unknown")
                    message = json_response.get("message", "")
                    print(f"✅ PostID: {post_id} | Status: {response.status_code} | Response: {status} - {message}")
                except Exception:
                    print(f"✅ PostID: {post_id} | Status: {response.status_code} | Raw: {response.text[:120]}")
                return True

            elif response.status_code == 403:
                consecutive_403_counter[0] += 1
                print(f"🚫 PostID: {post_id} | 403 Access Denied (consecutive: {consecutive_403_counter[0]})")
                if consecutive_403_counter[0] >= MAX_CONSECUTIVE_403:
                    print(f"❌ Too many consecutive 403s ({MAX_CONSECUTIVE_403}). Stopping to avoid IP block.")
                    shutdown_requested = True
                    return False
                wait_time = (2 ** attempt) * 5 + random.uniform(0, 5)
                print(f"⏳ Waiting {wait_time:.1f}s before retry...")
                time.sleep(wait_time)
                continue

            elif response.status_code == 429:
                wait_time = (2 ** attempt) * 2 + random.uniform(1, 3)
                print(f"⏳ PostID: {post_id} | Rate limited (429). Waiting {wait_time:.1f}s...")
                time.sleep(wait_time)
                continue

            elif response.status_code >= 500:
                wait_time = 2 ** attempt + random.uniform(0, 2)
                print(f"🔄 PostID: {post_id} | Server error {response.status_code}. Retry in {wait_time:.1f}s...")
                time.sleep(wait_time)
                continue

            else:
                print(f"❌ PostID: {post_id} | Failed with status: {response.status_code} | {response.text[:120]}")
                return False

        except requests.exceptions.Timeout:
            wait_time = 2 ** attempt + random.uniform(1, 3)
            print(f"⏱️  PostID: {post_id} | Timeout. Retry in {wait_time}s...")
            time.sleep(wait_time)

        except requests.exceptions.ConnectionError:
            wait_time = 2 ** attempt + random.uniform(1, 5)
            print(f"🔌 PostID: {post_id} | Connection error. Retry in {wait_time:.1f}s...")
            time.sleep(wait_time)

        except Exception as e:
            print(f"❌ PostID: {post_id} | Unexpected error: {e}")
            return False

    print(f"❌ PostID: {post_id} | Failed after {MAX_RETRIES} retries")
    return False

# ================== MAIN ==================
def main():
    global shutdown_requested

    print("=" * 65)
    print("🚀 NaMo Auto Like Reels Script (Exact Request Mimic)")
    print("=" * 65)
    print(f"📌 API URL     : {API_URL}")
    print(f"📌 Post Range  : {START_POST_ID} → {END_POST_ID}")
    print(f"📌 Workers     : {CONCURRENT_WORKERS}")
    print(f"📌 Delay/Req   : {DELAY_PER_REQUEST}s (+ jitter)")
    print(f"📌 Pause Every : {PAUSE_INTERVAL} requests for {PAUSE_DURATION}s")
    print(f"📌 Max Retries : {MAX_RETRIES} | Timeout: {REQUEST_TIMEOUT}s")
    print(f"📌 Consec 403s : {MAX_CONSECUTIVE_403} (stop)")
    print(f"📌 Proxy       : {'ON → ' + PROXY_URL if USE_PROXY else 'OFF'}")
    print("=" * 65)
    print()

    total_processed = 0
    successful = 0
    failed = 0
    consecutive_403 = [0]
    start_time = time.time()

    with requests.Session() as session:
        # We set headers per request, but session can hold default headers if any
        # Not setting session.headers because we use per-request headers

        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENT_WORKERS) as executor:
                futures = {}

                for post_id in range(START_POST_ID, END_POST_ID + 1):
                    if shutdown_requested:
                        break

                    future = executor.submit(send_request_with_retry, post_id, session, consecutive_403)
                    futures[future] = post_id

                    if len(futures) >= CONCURRENT_WORKERS * 2:
                        done, _ = concurrent.futures.wait(
                            futures,
                            return_when=concurrent.futures.FIRST_COMPLETED,
                        )
                        for completed_future in done:
                            try:
                                if completed_future.result():
                                    successful += 1
                                else:
                                    failed += 1
                            except Exception:
                                failed += 1
                            del futures[completed_future]
                            total_processed += 1

                    if total_processed > 0 and total_processed % PAUSE_INTERVAL == 0:
                        elapsed = time.time() - start_time
                        rate = total_processed / elapsed if elapsed > 0 else 0
                        print(
                            f"\n{'─' * 65}\n"
                            f"📊 Progress: {total_processed} processed "
                            f"({successful} ✅ | {failed} ❌) "
                            f"| Speed: {rate:.1f} req/s\n"
                            f"⏸️  Pausing for {PAUSE_DURATION}s...\n"
                            f"{'─' * 65}\n"
                        )
                        time.sleep(PAUSE_DURATION)

                    time.sleep(DELAY_PER_REQUEST)

                for future in concurrent.futures.as_completed(futures):
                    if shutdown_requested:
                        break
                    try:
                        if future.result():
                            successful += 1
                        else:
                            failed += 1
                    except Exception:
                        failed += 1
                    total_processed += 1

        except Exception as e:
            print(f"\n❌ Unexpected fatal error: {e}")

    elapsed = time.time() - start_time

    print()
    print("=" * 65)
    print("📊 FINAL SUMMARY")
    print("=" * 65)
    print(f"   Total Processed : {total_processed}")
    print(f"   Successful      : {successful} ✅")
    print(f"   Failed          : {failed} ❌")
    print(f"   Time Elapsed    : {elapsed:.1f}s")
    if elapsed > 0:
        print(f"   Avg Speed       : {total_processed / elapsed:.1f} req/s")
    print("=" * 65)

    if shutdown_requested:
        print("🛑 Process was stopped by user/system signal or due to too many 403s.")

if __name__ == "__main__":
    main()
